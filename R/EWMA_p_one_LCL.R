#' @title The one-sided lower control limit of an EWMA-p chart
#'
#' @description This function is used to calculate the one-sided lower control limit for EWMA-p charts with the correction of measurement error effects.
...
#'
#' \code{hat_ARL0}\eqn{\hspace{1.1cm}} The estimated in-control average run length based on given \code{L2}.
#'
#' \code{hat_MRL0}\eqn{\hspace{1.1cm}} The estimated in-control median of run length based on given \code{L2}.
#'
#' \code{hat_SDRL0}\eqn{\hspace{0.9cm}} The estimated in-control standard deviation of run length based on given \code{L2}.
#'
...
EWMA_p_one_LCL = function(p,lambda,n,pi1 = 1,pi2 = pi1,ARL0 = 200,M = 500,error = 10) {
  p0 = p
  p = (p+pi2-1)/(pi1+pi2-1)
  ...
  return(list(L2 = L$root, hat_ARL0 = hat_ARL0$ARL,
